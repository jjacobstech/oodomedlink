      ...
      <AuthenticatedLayout>
            <div class="w-full h-full bg-gray-100">
                  <div class="p-6 overflow-y-scroll">
                        <!-- Header -->
                        <div class="mb-6">
                              <h1 class="text-3xl font-bold text-gray-800">Patient Emails</h1>
                              <p class="text-gray-600">Monitor and manage all sent emails to patients</p>
                        </div>
                        ...
                        <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
                              <div class="flex gap-2">
                                    <button @click="selectedFilter = 'all'"
                                          ...
                                          class="px-4 py-2 font-medium rounded-lg transition-colors">All</button>
                                    <button @click="selectedFilter = 'in_progress'"
                                          :class="selectedFilter === 'in_progress' ? 'bg-orange-600 text-white' : 'bg-gray-200 text-gray-700'"
                                          class="px-4 py-2 font-medium rounded-lg transition-colors">In
                                          Progress</button>
                                    <button @click="selectedFilter = 'sent'"
                                          :class="selectedFilter === 'sent' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'"
                                          class="px-4 py-2 font-medium rounded-lg transition-colors">Sent</button>
                                    <button @click="selectedFilter = 'failed'"
                                          :class="selectedFilter === 'failed' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700'"
                                          class="px-4 py-2 font-medium rounded-lg transition-colors">Failed</button>
                              </div>
                              ...
                              <table class="w-full text-sm">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                          <tr v-for="email in filteredEmails" :key="email.id" class="hover:bg-gray-50">
                                                <td class="px-4 py-3 font-medium text-gray-800">{{ email.patient_name }}
                                                </td>
                                                <td class="px-4 py-3 text-gray-700">{{ email.email }}</td>
                                                <td class="px-4 py-3 text-gray-700 truncate max-w-[200px]">{{
                                                      email.subject }}</td>
                                                <td class="px-4 py-3 text-gray-600">{{ formatDate(email.sent_at) }}</td>
                                                <td class="px-4 py-3">
                                                      <span :class="{
                        ...
                        <div v-if="previewEmail"
                              ...
                              <div class="w-full max-w-2xl p-6 bg-white rounded-xl">
                                    <div class="flex items-center justify-between mb-4">
                                          <h3 class="text-2xl font-bold text-gray-800">{{ previewEmail.subject }}</h3>
                                          <button @click="previewEmail = null"
                                                class="px-4 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300">Close</button>
                                    </div>
                                    <div class="mb-4 text-sm text-gray-600">
                                          <p><strong>To:</strong> {{ previewEmail.patient_name }} ({{ previewEmail.email
                                                }})</p>
                                          <p><strong>Sent:</strong> {{ formatDate(previewEmail.sent_at) }}</p>
