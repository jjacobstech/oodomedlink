...
<template>
      ...
      <AuthenticatedLayout>
            <div class="w-full h-full bg-gray-100 px-10 pt-5">
                  <div class="p-6 overflow-y-scroll">
                        <!-- Header -->
                        <div class="flex justify-between">
                              <div class="mb-6">
                                    <h1 class="text-3xl font-bold text-gray-800">Patients Management</h1>
                                    <p class="text-gray-600">View and manage all registered patients and their test
                                          results
                                    </p>
                              </div>
                              <div>
                                    <button
                                          class="btn rounded-md bg-primary py-7 hover:-translate-y-1 duration-150 transition-all px-10 text-2xl text-white">
                                          Add Patient
                                    </button>
                              </div>
                        </div>
                        ...
                        <div class="flex flex-col gap-4 mb-6 md:flex-row md:items-center md:justify-between">
                              <div class="flex gap-2">
                                    ...
                                    <button @click="selectedFilter = 'Male'"
                                          :class="selectedFilter === 'Male' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'"
                                          class="px-4 py-2 font-medium rounded-lg transition-colors">Male</button>
                                    <button @click="selectedFilter = 'Female'"
                                          :class="selectedFilter === 'Female' ? 'bg-purple-600 text-white' : 'bg-gray-200 text-gray-700'"
                                          class="px-4 py-2 font-medium rounded-lg transition-colors">Female</button>
                              </div>
                              ...
                              <table class="w-full text-sm">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                          <tr v-for="patient in filteredPatients" :key="patient.id"
                                                ...
                                                <td class="px-4 py-3 text-gray-700">{{ patient.email }}</td>
                                                <td class="px-4 py-3 text-gray-700">{{ patient.phone }}</td>
                                                <td class="px-4 py-3 text-gray-700">{{ patient.gender }}</td>
                                                <td class="px-4 py-3 text-gray-700">{{ formatDate(patient.date_of_birth)
                                                      }}</td>
                                                <td class="px-4 py-3 text-gray-700">{{ patient.address }}</td>
                                                <td class="px-4 py-3 text-gray-700 text-center">{{ patient.total_results
                                                      }}</td>
                                                <td class="px-4 py-3 text-gray-600">{{
                                                      formatDate(patient.last_result_date) }}</td>
                                                <td class="px-4 py-3">
