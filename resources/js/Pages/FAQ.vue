<template>
      <div class="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50">
            ...
            <section class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                  <div class="relative">
                        <input v-model="searchQuery" type="text" placeholder="Search for help articles..."
                              class="w-full px-6 py-4 pr-12 rounded-xl border-2 border-slate-200 focus:border-primaryDark focus:outline-none text-xl shadow-sm" />
                        <svg class="absolute right-4 top-1/2 transform -translate-y-1/2 w-6 h-6 text-slate-400"
            ...
            <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                  ...
                  <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <button v-for="quick in quickLinks" :key="quick.title"
                              ...
                              class="bg-white p-6 rounded-xl shadow-sm hover:shadow-md transition text-left group">
                              <div
                                    :class="['w-12 h-12 rounded-lg mb-4 flex items-center justify-center', quick.bgColor]">
                                    <span class="text-2xl">{{ quick.icon }}</span>
                              </div>
                              <h3 class="font-semibold text-slate-900 group-hover:text-primaryDark transition">{{
                                    quick.title }}</h3>
                              <p class="text-lg text-slate-600 mt-1">{{ quick.description }}</p>
                        </button>
                  </div>
            </section>
                        ...
                        <div v-for="article in filteredArticles" :key="article.id"
                              ...
                              <button @click="toggleArticle(article.id)"
                                    ...
                                    <div class="flex items-center gap-4 flex-1">
                                          <div
                                                :class="['w-10 h-10 rounded-lg flex items-center justify-center text-xl', article.iconBg]">
                                                {{ article.icon }}
                                          </div>
                                          <div class="flex-1">
                                                <h3 class="font-semibold text-slate-900 text-xl">{{ article.title }}
                                                </h3>
                                                <p class="text-lg text-slate-600">{{ article.subtitle }}</p>
                                          </div>
                                    </div>
                                    <svg :class="['w-5 h-5 text-slate-400 transition-transform', expandedArticles.includes(article.id) ? 'rotate-180' : '']"
                                          fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                    </svg>
                              ...
                              <div v-if="expandedArticles.includes(article.id)"
                                    ...
                                    <div class="pt-4 space-y-4">
                                          <div v-for="(step, index) in article.steps" :key="index" class="flex gap-4">
                                                <div
                                                      class="flex-shrink-0 w-8 h-8 bg-blue-100 text-primaryDark rounded-full flex items-center justify-center font-semibold text-lg">
                                                      {{ index + 1 }}
                                                </div>
                                                <div class="flex-1">
                                          ...
                                          <div v-if="article.tips"
                                                class="bg-amber-50 border border-amber-200 rounded-lg p-4 mt-4">
                                                <div class="flex gap-2">
                                                      <span class="text-xl">💡</span>
                                                      <div>
                                                            <h4 class="font-semibold text-amber-900 mb-1">Pro Tip</h4>
                                                            <p class="text-lg text-amber-800">{{ article.tips }}</p>
                                                      </div>
                                                </div>
                                          </div>
                                    </div>
            ...
            <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 pb-16">
                  <div class="bg-primaryDark rounded-2xl p-8 text-white text-center">
                        ...
                        <div class="flex flex-wrap justify-center gap-4">
                              <a href="mailto:support@oodomedlink.com"
                                    class="bg-white text-primaryDark px-6 py-3 rounded-lg hover:bg-blue-50 transition font-bold">
                                    📧 Email Support
                              </a>
                              <button disabled
                                    class="bg-primaryDark text-white px-6 py-3 rounded-lg hover:bg-blue-400 transition font-bold">
                                    💬 Live Chat
                              </button>
                              <button disabled
                                    class="bg-primaryDark text-white px-6 py-3 rounded-lg hover:bg-blue-400 transition font-bold">
