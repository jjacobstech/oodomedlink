import { defineComponent, unref, mergeProps, withCtx, renderSlot, useSSRContext, createVNode, computed, ref, createTextVNode, toDisplayString, isVNode, resolveDynamicComponent, createBlock, openBlock, createCommentVNode, Fragment, renderList } from "vue";
import { ssrRenderComponent, ssrRenderSlot, ssrRenderList, ssrInterpolate, ssrRenderVNode, ssrRenderAttrs, ssrRenderAttr } from "vue/server-renderer";
...
import { X, EyeIcon, EyeClosed } from "lucide-vue-next";
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(ssrRenderComponent(unref(ToastClose), mergeProps(unref(delegatedProps), {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(ssrRenderComponent(unref(X), { class: "h-4 w-4 text-deepblue hover:text-white hover:bg-deepblue rounded-full" }, null, _parent2, _scopeId));
          } else {
            return [
              createVNode(unref(X), { class: "h-4 w-4 text-deepblue hover:text-white hover:bg-deepblue rounded-full" })
            ];
          }
        }),
        _: 1
                  ...
                  if (_push3) {
                    ...
                    return [
                      createVNode("div", { class: "grid gap-1" }, [
                        toast2.title ? (openBlock(), createBlock(unref(_sfc_main$3), { key: 0 }, {
                          default: withCtx(() => [
                            createTextVNode(toDisplayString(toast2.title), 1)
                          ]),
                          _: 2
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "border border-gray-300 rounded-lg focus-within:ring-2 focus-within:ring-deepgreen focus-within:border-transparent text-xl transition-all flex items-center" }, _attrs))}><input${ssrRenderAttr("value", __props.modelValue)}${ssrRenderAttr("type", passwordVisible.value ? "text" : "password")}${ssrRenderAttr("placeholder", __props.placeholder)} class="px-4 py-2.5 border-none focus:outline-none focus:ring-0 w-[85%] text-xl rounded-l-lg" required><div class="flex items-center justify-center w-[15%] px-auto">`);
