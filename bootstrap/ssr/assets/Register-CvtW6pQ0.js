...
import { _ as _sfc_main$1 } from "./AdminAuthLayout-CRRAGMZ_.js";
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            if (currentStep.value === 2) {
              _push2(`<form class="space-y-4"${_scopeId}><div class="space-y-2"${_scopeId}><label for="otp" class="text-sm font-medium"${_scopeId}>Verification Code</label><p class="mb-2 text-xs text-gray-500"${_scopeId}> Enter the 6-digit code sent to ${ssrInterpolate(unref(emailForm).email)}</p><input id="otp" type="text" placeholder="Enter 6-digit code" maxlength="6"${ssrRenderAttr("value", unref(otpForm).otp)} class="focus:ring-deepgreen w-full rounded-md border border-gray-300 px-3 py-2 text-center text-2xl tracking-widest focus:outline-none focus:ring-2"${_scopeId}></div><button type="submit" class="bg-deepgradient hover:shadow-deepgreen/30 w-full rounded-lg px-4 py-2.5 font-medium text-white transition-all duration-200 hover:-translate-y-1 hover:bg-white hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"${ssrIncludeBooleanAttr(isLoading.value) ? " disabled" : ""}${_scopeId}>${ssrInterpolate(isLoading.value ? "Verifying..." : "Verify Code")}</button><div class="mt-4 text-center"${_scopeId}><button type="button" class="text-deepgreen hover:text-deepgreen/80 text-sm transition-colors"${ssrIncludeBooleanAttr(isLoading.value) ? " disabled" : ""}${_scopeId}> Resend Code </button></div></form>`);
            } else {
              _push2(`<!---->`);
            }
            if (currentStep.value === 3) {
              ...
              ssrRenderList(biodataFields.value, (field) => {
                ...
                if (field.type !== "password") {
                  _push2(`<input${ssrRenderAttr("id", field.id)}${ssrRenderAttr("type", field.type)}${ssrRenderAttr("placeholder", field.placeholder)}${ssrRenderAttr("autocomplete", field.autocomplete)}${ssrRenderDynamicModel(field.type, unref(signupForm)[field.model], null)} class="focus:ring-deepgreen w-full rounded-md border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2"${_scopeId}>`);
                } else {
              ...
              _push2(`<!--]--><button type="submit" class="bg-deepgradient hover:shadow-deepgreen/30 w-full rounded-lg px-4 py-2.5 font-medium text-white transition-all duration-200 hover:-translate-y-1 hover:bg-white hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"${ssrIncludeBooleanAttr(isLoading.value) ? " disabled" : ""}${_scopeId}>${ssrInterpolate(isLoading.value ? "Signing up..." : "Complete Registration")}</button></form>`);
            } else {
              _push2(`<!---->`);
            }
            _push2(`</div></div></div>`);
            ...
            return [
              ...
              createVNode("div", { class: "flex h-screen items-center justify-center overflow-y-hidden" }, [
                createVNode("div", { class: "max-w-lg items-center justify-center rounded-xl bg-white shadow-xl" }, [
                  createVNode("div", { class: "bg-deepgradient grid space-y-1 rounded-t-lg p-6" }, [
