...
import { A as AuthenticatedLayout } from "./AuthenticatedLayout-DUXAsF_C.js";
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            ...
            ssrRenderList(__props.patients.data, (patient) => {
              _push2(`<div class="bg-white rounded-xl shadow-md p-4 space-y-3"${_scopeId}><div class="flex justify-between items-start"${_scopeId}><div class="flex-1 min-w-0"${_scopeId}><h3 class="font-semibold text-gray-800 truncate"${_scopeId}>${ssrInterpolate(patient.full_name)}</h3><p class="text-sm text-gray-600 truncate"${_scopeId}>${ssrInterpolate(patient.email)}</p></div><span class="inline-flex items-center justify-center rounded-full bg-blue-100 px-2.5 py-1 text-xs font-semibold text-blue-800 ml-2"${_scopeId}>${ssrInterpolate(patient.results_count)}</span></div><div class="grid grid-cols-2 gap-2 text-xs text-gray-600"${_scopeId}><div${_scopeId}><span class="font-medium"${_scopeId}>Phone:</span> ${ssrInterpolate(patient.phone_no || "N/A")}</div><div${_scopeId}><span class="font-medium"${_scopeId}>Gender:</span> ${ssrInterpolate(patient.gender || "N/A")}</div><div${_scopeId}><span class="font-medium"${_scopeId}>DOB:</span> ${ssrInterpolate(formatDate(patient.date_of_birth))}</div></div><div class="flex gap-2 pt-2"${_scopeId}><button class="flex-1 rounded-lg bg-blue-100 px-3 py-2 text-sm font-medium text-blue-700 transition hover:bg-blue-200"${_scopeId}> View </button><button class="flex-1 rounded-lg bg-green-100 px-3 py-2 text-sm font-medium text-green-700 transition hover:bg-green-200"${_scopeId}> Edit </button></div></div>`);
            });
            _push2(`<!--]-->`);
