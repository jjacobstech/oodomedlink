    ...
    const filteredPatients = computed(() => {
      ...
      if (searchQuery.value) {
        ...
        filtered = filtered?.filter(
          (p) => p.full_name.toLowerCase().includes(query) || p.email.toLowerCase().includes(query) || p.phone_no.toLowerCase().includes(query)
        );
      }
      return filtered;
    });
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="flex w-full gap-1 h-full overflow-y-hidden bg-primaryLight"${_scopeId}><div class="overflow-y-scroll w-full p-6"${_scopeId}><div class="flex justify-between"${_scopeId}><div class="mb-6"${_scopeId}><h1 class="text-3xl font-bold text-gray-800"${_scopeId}> Patients Management </h1><p class="text-gray-600"${_scopeId}> View and manage all registered patients and their test results </p></div><div${_scopeId}><button class="btn rounded-md bg-primaryDark px-10 py-7 text-2xl text-white transition-all duration-150 hover:-translate-y-1"${_scopeId}> Add Patient </button></div></div><div class="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between"${_scopeId}><div class="flex gap-2"${_scopeId}><button class="${ssrRenderClass([
              ...
              "rounded-lg px-4 py-2 font-medium transition-colors"
            ])}"${_scopeId}> Female </button></div><div class="relative"${_scopeId}><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Search by name, email or phone..." class="w-full rounded-lg border border-gray-300 px-4 py-2 pl-10 focus:border-transparent focus:ring-2 focus:ring-primaryDark md:w-80"${_scopeId}><span class="absolute left-3 top-1/2 -translate-y-1/2 transform text-gray-400"${_scopeId}>🔍</span></div></div><div class="overflow-x-auto rounded-xl bg-white shadow-md"${_scopeId}><table class="w-full text-sm"${_scopeId}><thead class="bg-gray-50"${_scopeId}><tr${_scopeId}><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Name </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Email </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Phone </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Gender </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Date of Birth </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Address </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Total Results </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Last Result </th><th class="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-700"${_scopeId}> Actions </th></tr></thead><tbody class="divide-y divide-gray-200"${_scopeId}><!--[-->`);
            ssrRenderList(filteredPatients.value, (patient) => {
              _push2(`<tr class="hover:bg-gray-50"${_scopeId}><td class="px-4 py-3 font-medium text-gray-800"${_scopeId}>${ssrInterpolate(patient.full_name)}</td><td class="px-4 py-3 text-gray-700"${_scopeId}>${ssrInterpolate(patient.email)}</td><td class="px-4 py-3 text-gray-700"${_scopeId}>${ssrInterpolate(patient.phone_no ?? "N/A")}</td><td class="px-4 py-3 text-gray-700"${_scopeId}>${ssrInterpolate(patient.gender ?? "N/A")}</td><td class="px-4 py-3 text-gray-700"${_scopeId}>${ssrInterpolate(patient.date_of_birth ? formatDate(patient.date_of_birth) : "N/A")}</td><td class="px-4 py-3 text-gray-700"${_scopeId}>${ssrInterpolate(patient.address ?? "N/A")}</td><td class="px-4 py-3 text-center text-gray-700"${_scopeId}>${ssrInterpolate(patient.results_count ?? "N/A")}</td><td class="px-4 py-3 text-gray-600"${_scopeId}>${ssrInterpolate(formatDate(patient.latest_result[0].uploaded_at) ?? "N/A")}</td><td class="px-4 py-3"${_scopeId}><button class="rounded bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 transition hover:bg-blue-200"${_scopeId}> Edit </button></td></tr>`);
            });
            _push2(`<!--]-->`);
                ...
                createVNode("div", { class: "overflow-y-scroll w-full p-6" }, [
                  createVNode("div", { class: "flex justify-between" }, [
                    ...
                    createVNode("div", null, [
                      createVNode("button", { class: "btn rounded-md bg-primaryDark px-10 py-7 text-2xl text-white transition-all duration-150 hover:-translate-y-1" }, " Add Patient ")
                    ])
                  ]),
                      ...
                      createVNode("tbody", { class: "divide-y divide-gray-200" }, [
                        (openBlock(true), createBlock(Fragment, null, renderList(filteredPatients.value, (patient) => {
                          return openBlock(), createBlock("tr", {
                            ...
                            createVNode("td", { class: "px-4 py-3 text-center text-gray-700" }, toDisplayString(patient.results_count ?? "N/A"), 1),
                            createVNode("td", { class: "px-4 py-3 text-gray-600" }, toDisplayString(formatDate(patient.latest_result[0].uploaded_at) ?? "N/A"), 1),
